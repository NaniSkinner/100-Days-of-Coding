  ...
  for (let i = 0; i < gridHeigth.value; i++) {
    ...
    for (let j = 0; j < gridWidth.value; j++) {
      ...
      col.addEventListener(events[deviceType].move, (e) => {
        let getElementById = document.elementFromPoint(
          !isTouchDevice() ? e.clientX : e.touches[0].clientX,
          !isTouchDevice() ? e.clientY : e.touches[0].clientY
        ).id;
        checker(getElementById);
      });
...
gridHeigth.addEventListener("input", () => {
  widthValue.innerHTML =
    gridHeigth.value < 9 ? `0${gridHeigth.value}` : gridHeigth.value;
});
...
//         // Use the correct variable for the function call
//         checker(getElementById);
//       });
